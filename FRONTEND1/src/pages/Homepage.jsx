...
import { FiShoppingCart, FiPlus, FiMinus, FiX, FiRefreshCw, FiSearch } from "react-icons/fi";
...
const CreateSale = () => {
  ...
  useEffect(() => {
    if (selectedProduct) {
      setQuantity(1);
      setSellingCost(selectedProduct.cost);
    }
  }, [selectedProduct]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setQuantity(1);
      setSellingCost(0);
      setSearchTerm(""); // Clear search after successful sale
    } catch (error) {
  ...
  const handleCategoryClick = (category) => {
    if (zoomedCategory && zoomedCategory._id === category._id) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Categories Grid */}
        <motion.div
          className="mb-10 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          {categories.map((cat, index) => (
            <motion.div
              key={cat._id}
              className={`cursor-pointer p-4 rounded-xl flex flex-col items-center justify-center bg-gradient-to-br ${getCategoryColor(index)} hover:shadow-xl transform transition-all duration-300 ${
                selectedCategory === cat._id ? "ring-2 ring-white ring-opacity-50 scale-105" : ""
              }`}
            ...
            >
              <motion.div
                className="w-20 h-20 rounded-xl mb-3 overflow-hidden border-2 border-white border-opacity-20"
              ...
              <p className="font-semibold text-center text-white">{cat.name}</p>
              <p className="text-xs text-white text-opacity-70 mt-1">
                {products.filter(p => p.category?._id === cat._id).length} alaab
              </p>
            </motion.div>
          ))}
        </motion.div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
              <div>
                <h2 className="text-2xl font-bold bg-gradient-to-r from-emerald-300 to-teal-300 bg-clip-text text-transparent">
                  {categories.find((c) => c._id === selectedCategory)?.name} - Alaabooyinka
                </h2>
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-12 bg-gray-700 rounded-xl">
                ...
                <p className="text-gray-400 text-lg">
                  {searchTerm ? 
                    `Wax alaab ah lagama helin "${searchTerm}"` : 
